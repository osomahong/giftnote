...
- `gift_` 접두사 맞춤 이벤트 컨벤션 준수

### 2. 매개변수 네이밍 검증
- GA4 예약 매개변수와 충돌 여부 (event_name, user_id 등)
- snake_case 준수, 40자 이내
- 이벤트당 매개변수 25개 이내
...
### 5. GA4 맞춤정의 매칭
- 맞춤 매개변수가 모두 GA4 맞춤 측정기준으로 등록되었는지 (Admin API로 조회)
- 미등록 매개변수는 GA4 보고서에서 보이지 않으므로 critical 경고

### 6. 데이터 품질
- 동일 액션에 중복 이벤트 발화 가능성 (예: 버블링으로 인한 이중 push)
- 카디널리티 경고: 맞춤 측정기준 값의 예상 고유값이 500+ 이면 경고
- 필수 전환 포인트 누락 (구매 클릭이 추적되지 않으면 critical)

### 7. 이전 버전 호환성
- 기존 이벤트명이 변경되었으면 경고 (히스토리컬 데이터 단절)
- 기존 매개변수명이 변경되었으면 경고
- 제거된 이벤트가 있으면 경고
